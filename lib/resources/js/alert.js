function connectpx_bookingAlert(alert) {
    ...
    if (not_empty) {
        let $container = jQuery('#connectpx_booking-alert');
        if ($container.length === 0) {
            $container = jQuery('<div id="connectpx_booking-alert" class="connectpx_booking-alert" style="max-width:600px"></div>').appendTo('#connectpx_booking_tbs');
        }
        for (let type in alert) {
