...
class Codes extends Order\Codes
{
    ...
    public function prepareForItem( Item $item, $recipient )
    {
        ...
        if ( ! $this->order->hasPayment() ) {
            ...
            if ( Config::taxesActive() && get_option( 'connectpx_booking_taxes_in_price' ) == 'excluded' ) {
    ...
    public function prepareReplaceCodes( $replace_codes, $format )
    {
        ...
        $cancel_appointment_confirm_url = get_option( 'connectpx_booking_url_cancel_confirm_page_url' );
        $cancel_appointment_confirm_url = $this->appointment_token
            ? add_query_arg( 'connectpx_booking-appointment-token', $this->appointment_token, $cancel_appointment_confirm_url )
            : '';

        // Add replace codes.
        $replace_codes += array(
            'appointment_date'               => $this->appointment_start === null ? __( 'N/A', 'connectpx_booking' ) : Utils\DateTime::formatDate( $this->appointment_start ),
            'appointment_time'               => $this->appointment_start === null ? __( 'N/A', 'connectpx_booking' ) : ( $this->service_duration < DAY_IN_SECONDS ? Utils\DateTime::formatTime( $this->appointment_start ) : $this->appointment_start_info ),
            'appointment_end_date'           => $this->appointment_start === null ? __( 'N/A', 'connectpx_booking' ) : Utils\DateTime::formatDate( $this->appointment_end ),
            'appointment_end_time'           => $this->appointment_start === null ? __( 'N/A', 'connectpx_booking' ) : ( $this->service_duration < DAY_IN_SECONDS ? Utils\DateTime::formatTime( $this->appointment_end ) : $this->appointment_end_info ),
            'appointment_notes'              => $format == 'html' ? nl2br( $this->appointment_notes ) : $this->appointment_notes,
            'approve_appointment_url'        => $this->appointment_token ? admin_url( 'admin-ajax.php?action=connectpx_booking_approve_appointment&token=' . urlencode( Utils\Common::xorEncrypt( $this->appointment_token, 'approve' ) ) ) : '',
            'booking_number'                 => $this->booking_number,
            'cancellation_reason'            => $this->cancellation_reason,
            'cancel_appointment_url'         => $this->appointment_token ? admin_url( 'admin-ajax.php?action=connectpx_booking_cancel_appointment&token=' . $this->appointment_token ) : '',
            ...
            'reject_appointment_url'         => $this->appointment_token
                ? admin_url( 'admin-ajax.php?action=connectpx_booking_reject_appointment&token=' . urlencode( Utils\Common::xorEncrypt( $this->appointment_token, 'reject' ) ) )
                : '',
