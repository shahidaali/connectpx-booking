jQuery(function($) {
    'use strict';
    let
        $appointmentsList   = $('#connectpx_booking-appointments-list'),
    ...
    if (urlParts.length > 1) {
        ...
        $.each(ConnectpxBookingL10n.datatables.appointments.settings.filter, function (field, value) {
            if (value != '') {
     ...
     * Init table columns.
     */
    $.each(ConnectpxBookingL10n.datatables.appointments.settings.columns, function (column, show) {
        if (show) {
            switch (column) {
    ...
    $.each(ConnectpxBookingL10n.datatables.appointments.settings.order, function (_, value) {
    ...
    var dt = $appointmentsList.DataTable({
        ...
        drawCallback: function( settings ) {
            // $('[data-toggle="connectpx_booking-popover"]', $appointmentsList).on('click', function (e) {
            //     e.preventDefault();
        ...
        ajax: {
            ...
            type: 'POST',
            data: function (d) {
                return $.extend({action: 'connectpx_booking_get_appointments', csrf_token: ConnectpxBookingL10nGlobal.csrf_token}, {
                    filter: {
                        id: $idFilter.val(),
