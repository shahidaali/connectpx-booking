...
class Inputs
{
    ...
    public static function renderCheckBox( $label, $value = null, $checked = null, $attrs = array() )
    {
        $attributes = array_merge( array(
            'id'      => array_key_exists( 'id', $attrs ) ? $attrs['id'] : 'connectpx_booking-ch-' . mt_rand( 0, PHP_INT_MAX ),
    ...
    public static function renderRadioGroup( $label, $help, array $radios, $value, $attrs = array() )
    {
        if ( empty( $radios ) ) {
            $radios = array(
                //value => data
                0 => array( 'title' => __( 'Disabled', 'connectpx_booking' ) ),
                1 => array( 'title' => __( 'Enabled', 'connectpx_booking' ) ),
            );
        }
        ...
        foreach ( $radios as $r_value => $r_data ) {
            $attributes = array_merge( array(
                'id'      => array_key_exists( 'id', $attrs ) ? $attrs['id'] : 'connectpx_booking-ra-' . mt_rand( 0, PHP_INT_MAX ),
