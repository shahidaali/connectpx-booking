define('ace/mode/connectpx_booking', function(require, exports, module) {

    var oop = require("ace/lib/oop");
    var TextMode = require("ace/mode/text").Mode;
    var ConnectpxBookingHighlightRules = require("ace/mode/connectpx_booking_highlight_rules").ConnectpxBookingHighlightRules;

    var Mode = function() {
        this.HighlightRules = ConnectpxBookingHighlightRules;
    };
...
define('ace/mode/connectpx_booking_highlight_rules', function(require, exports, module) {

    var oop = require("ace/lib/oop");
    var TextHighlightRules = require("ace/mode/text_highlight_rules").TextHighlightRules;

    var ConnectpxBookingHighlightRules = function() {

        this.$rules = new TextHighlightRules().getRules();

        // Generate connectpx_booking highlight rules
        this.$rules['start'] = this.$rules['start'] || [];
        // Start loop
        this.$rules['start'].push({
            token: 'connectpx_booking_each',
            regex: '{#each (\\w+(?:\\.\\w+)*)\\s+as\\s+(\\w+)(?:\\s+delimited\\s+by\\s+"(.+?)")?\\s*}',
            merge: false,
        });
        // End loop
        this.$rules['start'].push({
            token: 'connectpx_booking_endeach',
            regex: '{/each}',
            merge: false,
        });
        // Start if
        this.$rules['start'].push({
            token: 'connectpx_booking_if',
            regex: '{#if (\\w+(?:\\.\\w+)*)(.+?)}',
            merge: false,
        });
        // End if
        this.$rules['start'].push({
            token: 'connectpx_booking_endif',
            regex: '{/if}',
            merge: false,
        });
        // Code
        this.$rules['start'].push({
            token: 'connectpx_booking_code',
            regex: '{(\\w+(?:\\.\\w+)*(?:\\#\\w+)*)}',
            merge: false,
        });
    }

    oop.inherits(ConnectpxBookingHighlightRules, TextHighlightRules);

    exports.ConnectpxBookingHighlightRules = ConnectpxBookingHighlightRules;
});

define('ace/mode/connectpx_booking_completer', function(require, exports, module) {
    exports.ConnectpxBookingCompleter = function( codes ) {
        ...
        return {
            ...
            getCompletions: function (state, session, pos, prefix, callback) {
                let iterator    = new TokenIterator(session, pos.row, pos.column),
                    // Get token for current cursor position (e.g. 'connectpx_booking_code' from highlight rules)
                    token       = iterator.getCurrentToken(),
                    completions = [];
                // Disable completions inside {#each ...}
                const line = session.getLine(pos.row);
                if (token && token.type === 'connectpx_booking_each' && line[pos.column - 1] !== '}' && line[pos.column] !== '{') {
                    callback(null, []);
                }
                // Fix issue when each or endeach starts at new line
                if (pos.column === 0 && token && (token.type === 'connectpx_booking_endeach' || token.type === 'connectpx_booking_each')) {
                ...
                if (codes) {
                    ...
                    while (token && !(token.type === 'connectpx_booking_each' && level === 0)) {
                        if (token.type === 'connectpx_booking_endeach') {
                            level++;
                        }
                        if (token.type === 'connectpx_booking_each') {
                            level--;
                        }
                        token = iterator.stepBackward();
                    }
